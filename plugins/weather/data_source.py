...
async def get_weather_of_city(city: str) -> str:
    ...
    if code == 999:
        ...
        key = f"f496fda3a9f506a83ca9aa738029970b"
        param = {'city': f'{str(code)}', 'key': 'f496fda3a9f506a83ca9aa738029970b', 'extensions': 'all'}
        url = f"https://restapi.amap.com/v3/weather/weatherInfo"
        data = requests.get(url, params=param).text
        forecasts = json.loads(data)['forecasts'][0]
        week_dict = {'1': '星期一', '2': '星期二', '3': '星期三', '4': '星期四', '5': '星期五', '6': '星期六', '7': '星期日', '0': '星期日'}
        casts = forecasts['casts']
        repass = f"{city}近期天气情况：\n"
        for weather in casts:
            week_num = weather['week']
            time = weather['date'] + ' ' + week_dict[week_num]
...
def get_city_list() -> List[list]:
    ...
    for p in data:
        ...
        for key, value in p.items():
            c.append(value)
        city_list.append(c)
    #[[北京,110000,xxxx],[东城区,110101,xxxx]]
    # global city_list
    return city_list


def _check_exists_city(city: str, city_list=None) -> int:
    """
    检测城市是否存在合法
    :param city: 城市名称
    """
...
# _check_exists_city('sf',get_city_list())
